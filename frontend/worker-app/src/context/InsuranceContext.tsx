...
type ProtectionState = 'Protected' | 'Claim Processing';
...
export const InsuranceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [protectionState, setProtectionState] = useState<ProtectionState>('Protected');
  ...
  useEffect(() => {
    ...
    if (isTriggered && !wasTriggered) {
      ...
      setProtectionState('Claim Processing');
      setAutoClaimNotification(
        `⚠️ Disruption Detected in ${zoneName}. Your Income Shield has been automatically activated. Estimated Payout: ₹${effectivePayout.toLocaleString('en-IN')}.`,
      );
    }
  ...
  }, [disruptionReport.triggered, disruptionReport.level, disruptionReport.rainMm, workerProfile.zone, workerProfile.city, payoutEstimate, shieldWallet]);
